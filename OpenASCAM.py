...
class Instructions: #les différentes fonctions d'instructions possibles pour usiner, percer...
	...
	def trou(self,chaine,nb_var,parametres): #fonction pour percer un trou
		...
		for i in range(1,int(parametres.liste[4][1])+1):
			...
			affleurement=(float(parametres.liste[4][1])-i+1)*float(parametres.liste[0][1])/float(parametres.liste[4][1])+float(parametres.liste[11][1])
...
liste_parametres=['ep_matiere','marge_z','f_deplacement','f_descente','z_passe_percer','off_x','off_y','accel','d_fraise','z_passe_decouper','f_decoupe','off_z']
